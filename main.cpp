...
EdgePtr InsertNodesIntoLCA(EdgePtr ptrLCA, const std::vector<Edge>& vectorSI, int i){
	...
	if(!vectorSI[i].isEmpty()){
		// add while vectorSI[i]->getNext() pour bien ajouter tous les noeuds ‡ cette valeur de y dans la LCA
...
/*EdgePtr RemoveNodesFromLCA(EdgePtr ptrLCA, int i){
	...
	// ajouter la conversion Y coordonnÈe viewport et openGL (les indices sont des entiers)
...
void FillingLCALoop(CPolygon const &polygon){
	...
	for(int i = 0 ; i < ET.size() ; i++){
		// vÈrifier si la ptrLCA est bien changÈe, ‡ voir si il ne faut pas que Insert la renvoie ou qu'on passe la prtLCA par valeur 
		// et non par copie
		/*
		if(vectorSI[i].isEmpty()){
			printf("Hello");
